...
public record UnsuspendUserCommand(Guid UserId, Guid ActorUserId) : IRequest<Result<Unit>>;

public class UnsuspendUserCommandHandler(
    IApplicationDbContext db,
    IAuditService audit)
    : IRequestHandler<UnsuspendUserCommand, Result<Unit>>
{
    public async Task<Result<Unit>> Handle(UnsuspendUserCommand req, CancellationToken ct)
    {
        ...
        await audit.LogAsync(
            ...
            "User",
            user.Id,
            new { before = new { previousIsActive, previousIsSuspended }, after = new { user.IsActive, user.IsSuspended } },
            ct);
